# Admin Mini App — UI/UX Redesign (v2, repo-verified)

**Date:** 2026-08-15
**Status:** Draft v2 — verified against `github.com/Zahed316/azadi` source
**Author:** Zahed + Claude
**Companion spec:** `2026-08-14-ai-admin-design.md` (AI chat + merged Inventory tab; ships this work)
**Supersedes:** `2026-08-15-admin-mini-app-ux-redesign.md` (v1) — this version resolves v1's open assumption #3, fixes two gaps v1 missed, and confirms the rest of v1 against actual code.
...
1. **One bottom nav serves two very different users.** `super_admin` sees 4 tabs (موجودی / آمار / تنظیمات / اطلاعات). `category_admin` sees 1 tab (موجودی). Confirmed in `App.tsx`: the nav renders exactly this split via `{isSuperAdmin && (...)}`. The roles are not just permission tiers — they have different _cadence_ and _intent_. The current UI forces both through the same surface.
...
The data model is a _major_ constraint that shapes everything: see [§3 Per-branch inventory mechanics](#3-per-branch-inventory-mechanics). Confirmed directly against `admin-app/src/api/types.ts`: products are scoped to a single branch (`Product.branchId: number | null`) with a single `stock: number` field — NOT a per-branch stock matrix. The redesign must accommodate both today's model and a future per-branch-stock migration without painting itself into a corner.
...
| ---------------- | ---------------------------------------------- | --------------------------------- | ------------ |
| `super_admin`    | `/inventory` (categories sub-tab, full editor) | موجودی / آمار / تنظیمات / اطلاعات | "Manage"     |
| `category_admin` | `/counter` (a NEW landing screen — see §4.2)   | پیشخوان / موجودی                  | "Toggle"     |
...
The chat panel should stay accessible to both roles per the AI's own role guard on `/api/ai/chat` (backend already refuses out-of-scope requests for `category_admin`).

> **⚠️ Verified gap — not currently true.** `ChatButton.tsx` has a hard client-side gate: `if (!isSuperAdmin) return null;`. The chat FAB is invisible to `category_admin` today regardless of what the backend allows. Making the chat reachable by both roles requires removing/relaxing this check, not just relying on the backend guard — see §7.1 and Phase 4 (§9).
...
### 3.2 The redesign must design for both

**Path A — Status quo is fine.** Many small F&B businesses duplicate their menu per branch (cold brew at branch A has its own row, cold brew at branch B has its own row, distinct names if needed). The redesign just needs to make this workflow obvious: a "branch" picker on the product create form, a branch badge on every product card, and a "Clone to other branch" action. **No schema change needed.**

**Path B — We want true per-branch stock for the same SKU.** This requires a schema migration: a new `product_stocks` table (`product_id`, `branch_id`, `quantity`, `available`) and changes to the stock/toggle endpoints. **Significant backend work.**

I recommend **Path A as the default**, with the UI designed to make the per-branch distinction unmissable, and Path B left as a flag-able future enhancement. **§8 (Open Questions) asks the owner to confirm.**
...
The two branch pills always show. Tapping a pill toggles `available` at that branch (Path A: toggles on the product, applies to that branch only via `branchId` matching; Path B: toggles the row in `product_stocks`). The stock number in the middle is editable inline on tap (see §5.3).
...
Tapping opens a sheet with two branch rows (`Branch` type confirmed in `types.ts`: `id`, `name`, `address`, `phone`, `location`, `openingHours`, `isActive`) + an "همه" option. The selection is local state, not stored — baristas might switch mid-shift.
...
The existing edit form in `ProductsSubTab.tsx` is fine in shape; the redesign fixes the _trigger_:
...
### 4.4 The Configure / Info consolidation (optional, do last)

`ConfigurePage` and `InfoPage` are both bag-of-cards pages today (Settings + MenuConfig + Admins in one; AboutUs + FAQ + Messages in another). The bottom nav has them as two separate tabs. Recommend merging into one tab called "تنظیمات" with sub-tabs "عمومی" (Settings, MenuConfig) / "محتوا" (FAQ, AboutUs) / "مدیریت" (Admins, Messages). Admins and Messages are very low-frequency — they deserve real estate only when intentionally visited, not a bottom-nav slot.

This is the lowest-priority change in this spec — it improves the bottom nav for owners but doesn't unblock any operational workflow.
...
v1 flagged the `@telegram-apps/sdk` v2 surface as unverified. I installed `@telegram-apps/sdk@^2.0.0` (matches `admin-app/package.json`) and inspected the real export list. **The affordances all exist, but v2's actual shape differs from v1's pseudocode** — it exports flat objects/signals, not classes:
...
`bindThemeParamsCssVars()` already does most of the CSS-variable wiring for you — no need to hand-roll it in the hook. `index.css` then just maps `var(--tg-theme-bg-color, ...)` etc. to the app's own token names (see §6.2, unchanged from v1).
...
<InlineStockEditor
  ...
  onZero={() => {
    ...
    haptics.error(); // maps to hapticFeedback.notificationOccurred('warning') via the wrapper
  }}
/>
...
> **Note:** extract `toggleProductField` out of `ProductsSubTab.tsx` into a shared `useProductMutations.ts` hook so `/counter` (new) and `/inventory` (existing) share one mutation surface instead of duplicating it. This is new refactor work not called out in v1 — add to Phase 3 (§9).

### 5.4 The Bottom Nav refinement

Today: `position: fixed` nav with `NavLink`s and emoji icons (confirmed in `App.tsx`). The redesign:

- **Drop to 3-4 visible tabs** at most. `category_admin` sees 2 (پیشخوان / موجودی). No horizontal overflow needed at that count.
...
- **The chat FAB moves into the nav** as the rightmost item instead of floating (`ChatButton.tsx` currently renders as a fixed-position FAB, confirmed). This frees screen real estate for content and aligns the FAB with the nav's safe-area.
  - **This requires removing `ChatButton.tsx`'s current `if (!isSuperAdmin) return null` gate** if the owner confirms §8 Q3 (chat visible to both roles) — otherwise `category_admin`'s 2-tab nav has nowhere to put a chat tab and the FAB simply doesn't render for them, same as today. See Phase 4 (§9).
...
**Replace `Inter` with `Vazirmatn` for Persian text + `Geist` for Latin glyphs.** Confirmed: `index.css` line 30–31 currently opens the font stack with `'Inter'`.
...
Replace the emoji nav icons (`📋` `📊` `⚙️` `ℹ️`) and the `🏷️` `📦` `🏠` `✉️` etc. scattered through the UI with a single inline SVG sprite file. **Confirmed no icon package is currently installed** — commit to inline SVG (`<use href="#icon-name">`) rather than adding Phosphor or any other dependency; this was left open in v1 and is now resolved.
...
| `admin-app/src/index.css`                     | Add Double-Bezel tokens. Replace Inter with Vazirmatn + Geist. Inline SVG icon system. New motion tokens.                                                                         |
...
| `admin-app/src/pages/InventoryPage.tsx`       | Add branch selector above category chips. **Fix `initialTab` to default to `'products'` for `category_admin`, `'categories'` for `super_admin`** (new — verified gap, not in v1). |
| `admin-app/src/components/ChatButton.tsx`     | Re-render as nav tab instead of floating FAB. **Remove/relax the `if (!isSuperAdmin) return null` gate** if §8 Q3 is confirmed (new — verified gap, not in v1).                   |

### 7.2 Create

| File                                             | Purpose                                                                                                                                              |
| ------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `admin-app/src/pages/CounterPage.tsx`            | Barista home screen.                                                                                                                                 |
...
| `admin-app/src/components/BranchSelector.tsx`    | Sticky branch picker pill.                                                                                                                           |
...
| `admin-app/src/hooks/useProductMutations.ts`     | Shared `toggleProductField`/stock mutation, extracted from `ProductsSubTab.tsx` so `/counter` and `/inventory` don't duplicate it (new — not in v1). |

### 7.3 Don't touch

- `src/api/resources/products.ts` — the API is fine for Path A (confirmed: `updateStock`/`toggleAvailability` endpoints already support everything the redesign needs). Path B (per-branch stock) would require new endpoints, deferred to a separate spec.
...
1. **Per-branch stock model — Path A or Path B?** See §3. Path A is the safer recommendation (zero migration, immediate UI wins). Path B requires a real schema migration and is a separate, larger project. Owner confirms which to commit to.
2. **Counter screen scope — baristas or all non-super-admins?** Today there are only two roles. If the owner plans a third role (e.g., "kitchen" with no edit access), the Counter routing should consider that. Confirm: `category_admin` is the only role that lands on `/counter`?
3. **Chat panel accessibility.** The 2026-08-14 spec scoped the AI to `super_admin` only, and `ChatButton.tsx`'s current client-side gate enforces that in the frontend too (confirmed — this isn't just a backend default). Should `category_admin` see the chat button at all? Recommend: yes, visible everywhere but AI returns a polite refusal for out-of-scope actions (consistent with the existing backend role guard). **If yes, this requires an actual code change to `ChatButton.tsx`, not just a config flip — see §7.1.**
4. **Branch context persistence.** Should the barista's selected branch persist across sessions, or default to "all branches" each shift start? Recommend: default to "all" — picking a branch is one tap and avoids showing yesterday's branch to today's barista.
5. **Configure + Info consolidation.** §4.4 proposes merging the two into one tab. Is the loss of one tap to enter a low-frequency page worth the cleaner nav? Owner decides.
6. **Font loading.** Vazirmatn CDN adds ~80KB of font weight. Acceptable trade for the aesthetic upgrade, or do you prefer to self-host? (Self-host in `admin-app/public/fonts/` is straightforward.)
...
- Add Vazirmatn + Geist, replace Inter. Pure CSS change, zero behavior change. Validate the rendering on Telegram light + dark themes.
...
- Replace the floating chat FAB with a nav tab (visual move only in this phase; the `isSuperAdmin` gate itself is addressed in Phase 4, contingent on §8 Q3).
...
- Extract `toggleProductField` out of `ProductsSubTab.tsx` into shared `useProductMutations.ts` (new step, not in v1 — needed so `/counter` in Phase 4 doesn't duplicate the mutation logic).
...
- Build `/counter` page for `category_admin`, reusing `useProductMutations.ts` from Phase 3.
- Add branch selector + search overlay + accordion list.
- Wire it as the default route for non-super-admin.
- Trim the bottom nav to 2 tabs for `category_admin`.
- **If §8 Q3 is confirmed:** remove/relax the `isSuperAdmin` gate in `ChatButton.tsx` and add the chat tab to `category_admin`'s now-2-tab (→3-tab) nav. This is the natural point to do it, since it's when `category_admin` first gets meaningful nav real estate.
...
### Phase 6 — Polish + per-branch (3-5 days, contingent on §8 Q1)

- If Path A: ship "Clone to other branch" action and ensure branch badges are obvious everywhere.
- If Path B: implement schema migration + new endpoints. Defer the UI work for this path.
...
3. ~~`@telegram-apps/sdk` v2 supports all the affordances listed in §5.1~~ — **Resolved.** Installed and inspected directly; the affordances exist under different names/shapes than v1's pseudocode assumed. See §5.1 for the corrected API.
...
6. **The product's `stock` of 0 is equivalent to "out of stock" but `available = false` is the _customer-visible_ signal.** The redesign's panic-zero action sets both. If the business treats stock > 0 with `available = false` as a meaningful state (e.g., "temporarily hidden, will return"), the panic action needs user confirmation.
...
1. A barista (`category_admin`) lands on `/counter`, can mark any product out of stock in 1 tap, and never sees `/configure` or `/info` in their nav.
...
3. `category_admin` opening `/inventory` directly lands on the `products` sub-tab, filtered to their `allowedCatId`, not the `categories` sub-tab (new — fixes the verified gap in §2.3/§7.1).
...
6. Bottom nav is 4 tabs max, never horizontally scrolls.
7. All transitions use the three custom cubic-beziers from §5.5 (no `linear`, no `ease-in-out`).
8. The app passes `npm run check` (typecheck + lint + format) and ships without bundle size regression (target: ≤ current `admin-app/dist` size + 100KB for Vazirmatn subset — no icon-library dependency added, per §6.3).
