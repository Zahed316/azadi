...
import { ProductRepository, BranchRepository, FaqRepository, CategoryRepository, SettingsRepository, MenuConfigRepository } from '../repositories';
  ...
  try {
    ...
    if (path === 'menu-config') {
      ...
      if (method === 'GET') {
        ...
        return new Response(JSON.stringify({ success: true, menuConfig: result[0] }), { headers: corsHeaders });
      }
    }
